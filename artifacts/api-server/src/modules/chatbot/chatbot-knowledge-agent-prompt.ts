...
export function buildKnowledgeAgentPrompt(opts: BuildKnowledgeAgentPromptOpts): string {
  ...
  return [
    ...
    "6. ЗАПРЕЩЕНО: «минутку», «сейчас проверю», «подождите» — проверка слота мгновенная на сервере.",
    "7. Если пациент назвал дату/время и врач уже подобран — ОБЯЗАТЕЛЬНО actions: [{\"type\":\"parse_datetime\",\"datetimeText\":\"...\"}].",
    "8. После parse_datetime сервер вернёт результат (свободно/занято) — отвечай по факту, не обещай проверку.",
    "9. fsmHint — только для аналитики (greeting, collect_problem, suggest_doctor, collect_datetime, done).",
    "",
