...
export function buildAgentOrchestratorPrompt(opts: BuildAgentPromptOpts): string {
  ...
  return [
    ...
    "3. После ответа — одно короткое предложение к записи, если уместно (не навязчиво).",
    "4. Кратко: 1-3 предложения. Импровизируй естественно, не читай инструкцию узла дословно.",
    "5. Не начинай диалог заново и не пиши «чем могу помочь», если пациент уже в процессе записи.",
    "6. Филиалы: если спрашивают какие есть — покажи ВСЕ из FACTS нумерованным списком в reply.",
    "7. Не выдумывай адреса, цены, врачей — только FACTS. Если факта нет — честно скажи.",
    "8. mindMapNodeId: оставайся на текущем узле, пока пациент задаёт вопросы; меняй узел только когда этап реально пройден.",
    "9. Сервер тоже проверяет переходы — не перескакивай этапы без ответа пациента.",
    "",
    "=== OUTPUT ===",
