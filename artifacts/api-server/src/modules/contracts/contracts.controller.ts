...
// ── Public (no-auth) contract viewer routes ───────────────────────────────

// GET /contracts/public/contract/:token
router.get(
  "/public/contract/:token",
  async (req: Request, res: Response, next: NextFunction) => {
    const token = String(req.params["token"]);
    const row = await repo.findContractByToken(token).catch(next);
    if (row === undefined) return;
    if (!row) return next(new NotFoundError("Договор не найден"));

    // Mark as viewed (only if currently "sent")
    await repo.markContractViewed(token).catch(() => {});
    ...
    res.json({
      ...
      data: {
        ...
        renderedHtml: row.contract.renderedHtml,
        templateName: row.templateName,
        patientName: row.patientName,
        clinicName: row.clinicName,
        signedAt: row.contract.signedAt,
      },
    });
  },
);

// POST /contracts/public/contract/:token/sign
router.post(
  "/public/contract/:token/sign",
  async (req: Request, res: Response, next: NextFunction) => {
    const token = String(req.params["token"]);
    const ip = req.ip ?? req.socket?.remoteAddress ?? null;
    const signed = await repo.markContractSigned(token, ip).catch(next);
    if (signed === undefined) return;
    if (!signed) return next(new NotFoundError("Договор не найден"));
    res.json({ success: true });
  },
);

// GET /contracts/public/bundle/:bundleToken
router.get(
  "/public/bundle/:bundleToken",
  async (req: Request, res: Response, next: NextFunction) => {
    const bundleToken = String(req.params["bundleToken"]);
    const rows = await repo.findContractsByBundleToken(bundleToken).catch(next);
    if (!rows) return;
    if (rows.length === 0) return next(new NotFoundError("Пакет не найден"));
...
// POST /contracts/public/bundle/:bundleToken/sign
router.post(
  "/public/bundle/:bundleToken/sign",
  async (req: Request, res: Response, next: NextFunction) => {
    const bundleToken = String(req.params["bundleToken"]);
    const ip = req.ip ?? req.socket?.remoteAddress ?? null;
    const signed = await repo.markBundleSigned(bundleToken, ip).catch(next);
    if (!signed) return;
    if (signed.length === 0) return next(new NotFoundError("Пакет не найден"));
    res.json({ success: true });
  },
);
