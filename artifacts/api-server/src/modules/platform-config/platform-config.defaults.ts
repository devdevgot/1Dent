...
/** Follow-up / objection / quality sections appended to every opus meta-prompt. */
export const OPUS_META_SECTION_FOLLOWUPS = `8. ПОВТОРНЫЕ КАСАНИЯ (FOLLOW-UP)
...
/** Pre-appointment visit confirmation section appended to every opus meta-prompt. */
export const OPUS_META_SECTION_VISIT_CONFIRM = `11. ПОДТВЕРЖДЕНИЕ ВИЗИТА ЗА ЧАС ДО ПРИЁМА
...
/**
 * Append the platform sections (8–11) to a stored meta-prompt that predates them.
 * Never removes or rewrites the stored text — sections are inserted before
 * «=== ВХОД ===» when present, otherwise appended at the end.
 */
export function mergePlatformSectionsIntoOpusMetaPrompt(prompt: string): string {
  const missing: string[] = [];
  if (!prompt.includes("ПОВТОРНЫЕ КАСАНИЯ")) missing.push(OPUS_META_SECTION_FOLLOWUPS);
  if (!prompt.includes("ПОДТВЕРЖДЕНИЕ ВИЗИТА ЗА ЧАС")) missing.push(OPUS_META_SECTION_VISIT_CONFIRM);
  if (missing.length === 0) return prompt;

  const addendum = missing.join("\n\n");
  const inputMarker = "=== ВХОД ===";
  const markerAt = prompt.indexOf(inputMarker);
  if (markerAt >= 0) {
    return `${prompt.slice(0, markerAt).trimEnd()}\n\n${addendum}\n\n${prompt.slice(markerAt)}`;
  }
...
export function resolveOpusMetaPrompt(stored?: string | null): string {
  ...
  if (text.includes("ФОРМАТ ОТВЕТОВ") || text.includes("replyParts")) {
    return mergePlatformSectionsIntoOpusMetaPrompt(text);
  }
