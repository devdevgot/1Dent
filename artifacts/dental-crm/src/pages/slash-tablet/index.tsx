...
export default function SlashTabletPage() {
  ...
  const [patient, setPatient] = useState<TabletPatient | null>(null);
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "patients" && doctor && (
          ...
          >
            ...
            <div className="flex-1 overflow-auto">
              <PatientList onSelect={(p) => { setPatient(p); setStep("card"); }} />
            </div>
          </motion.div>
        )}
        ...
        {step === "card" && patient && (
          <motion.div key="card" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <PatientCard patient={patient} onBack={() => setStep("patients")} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
