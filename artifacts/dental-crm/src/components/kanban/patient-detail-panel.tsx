...
export function PatientDetailPanel() {
  ...
  // Reset panel session state when switching patients (prevents stale plan/diagnosis UI).
  useEffect(() => {
  ...
  // Clear stale plan detail id if it doesn't belong to the current patient.
  useEffect(() => {
    if (!planDetailId || planLoading || plansLoading) return;
    const belongsToPatient =
      activePlan?.id === planDetailId ||
      allPlans.some((p) => p.id === planDetailId);
    if (!belongsToPatient) setPlanDetailId(null);
  ...
  const handleVoiceDiagnosisApplied = useCallback(async (result: VoiceDiagnosisApplyResult) => {
    ...
    if (!activePlan && result.servicesByTooth.size > 0) {
      ...
      if (items.length > 0) {
        const res = await createPlanMutation.mutateAsync({ id: selectedPatientId, data: { items } });
        const newPlanId = res?.data?.plan?.id;
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <svg width={sz} height={sz} className="shrink-0">
                                      ...
                                      {apPct > 0 && <circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke="hsl(var(--ds-primary))" strokeWidth="8"
                                        strokeDasharray={String(circ)} strokeDashoffset={String(offset)} strokeLinecap="round"
                                        transform={`rotate(-90 ${sz/2} ${sz/2})`} />}
                                      <text x={sz/2} y={sz/2 + 4} textAnchor="middle" fontSize="11" fontWeight="bold" fill="hsl(var(--ds-primary))">{apPct}%</text>
                                    </svg>
                                  );
                                })()}
                                <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-4 mb-4">
                                {(() => {
                                  ...
                                  return (
                                    <svg width={sz} height={sz} className="shrink-0">
                                      <circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke="#f1f5f9" strokeWidth="8" />
                                      {pct > 0 && <circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke="hsl(var(--ds-primary))" strokeWidth="8"
                                        strokeDasharray={String(circ)} strokeDashoffset={String(offset)} strokeLinecap="round"
                                        transform={`rotate(-90 ${sz/2} ${sz/2})`} />}
                                      <text x={sz/2} y={sz/2 + 4} textAnchor="middle" fontSize="11" fontWeight="bold" fill="hsl(var(--ds-primary))">{pct}%</text>
                                    </svg>
                                  );
                                })()}
                                <div className="flex-1 min-w-0">
