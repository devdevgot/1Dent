    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border/50">
          <h3 className="text-sm font-semibold">Воронка по этапам FSM</h3>
          <p className="text-xs text-muted-foreground mt-0.5">Сколько пациентов дошло до каждого шага</p>
        </div>
            ...
            .map((stage) => (
              <div key={stage.state} className="px-4 py-3 flex items-center gap-4">
                ...
                <div className="w-28 shrink-0">
                  ...
                  <p className="text-[10px] text-muted-foreground text-right mt-0.5">{stage.conversionRate}%</p>
                </div>
              </div>
            ))}
        </div>
      ...
      {analytics.variants.length > 0 && (
        <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border/50">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Сравнение вариантов по конверсии в запись</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {analytics.variants.map((v) => (
                  <tr key={v.variantId} className="border-b border-border/30 last:border-0">
                    <td className="px-4 py-2.5 font-medium">{v.variantName}</td>
                    <td className="px-4 py-2.5">{v.sessions}</td>
                    <td className="px-4 py-2.5">{v.bookings}</td>
                    ...
                    <td className="px-4 py-2.5 text-muted-foreground">{v.handoffs}</td>
                  </tr>
                ))}
              </tbody>
            </table>
