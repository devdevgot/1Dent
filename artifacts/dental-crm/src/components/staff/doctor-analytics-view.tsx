...
import type {
  DoctorDetailedAnalytics,
  DoctorDetailedAnalyticsRevenueByMonthItem,
  DoctorDetailedAnalyticsProceduresByNameItem,
...
export type AnalyticsPreset = "all" | "7d" | "30d" | "3m" | "6m" | "1y" | "custom";

export function toIsoDate(d: Date): string {
  return d.toISOString().split("T")[0]!;
}

export function dateMinusDays(d: Date, days: number): Date {
  const r = new Date(d);
  r.setDate(r.getDate() - days);
  return r;
}

export function dateMinusMonths(d: Date, months: number): Date {
  const r = new Date(d);
  r.setMonth(r.getMonth() - months);
  return r;
}

export function computeDateRange(preset: AnalyticsPreset, customFrom: string, customTo: string) {
  const now = new Date();
...
type DoctorAnalyticsViewProps = {
  analytics: DoctorDetailedAnalytics | undefined;
  isLoading: boolean;
  isFetching: boolean;
  isError: boolean;
  refetch: () => void;
  ...
  minRevenueInput: string;
  setMinRevenueInput: (v: string) => void;
...
export function DoctorAnalyticsView({
  ...
  if (isLoading) {
    return <DoctorAnalyticsContentSkeleton />;
  }
      ...
      {showFiltersButton && (
        <div className="px-5 pt-3 flex justify-end">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 text-sm font-semibold transition-all ${
              hasActiveFilters
...
export function DoctorAnalyticsFiltersBadge({
  preset,
  customFrom,
  customTo,
  selectedType,
