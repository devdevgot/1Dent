  ...
  const scrollBody = (
    <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-5">
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-[#e8e3d9] divide-y divide-[var(--ds-border)]">
              {(() => {
                const scheduledItems = planItems
                  .filter((p) => p.status === "pending" && p.scheduledAt)
                  .sort((a, b) => new Date(a.scheduledAt!).getTime() - new Date(b.scheduledAt!).getTime());
  ...
  if (isTablet) {
    return (
      <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
        <DialogContent
          hideClose
          className="flex max-h-[min(88dvh,900px)] w-[min(92vw,48rem)] max-w-3xl flex-col gap-0 overflow-hidden rounded-2xl border border-[#e8e3d9] bg-white p-0 shadow-xl"
        >
          <div className="flex items-center justify-between border-b border-[#e8e3d9] px-5 py-4 shrink-0">
            <DialogTitle className="text-base font-bold text-[#0f172a] pr-4">{stage.label}</DialogTitle>
            <button
              type="button"
              onClick={onClose}
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f1ede4] text-[#64748b] transition-colors hover:bg-[var(--ds-border)]"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
      ...
      >
        <div className="flex items-center justify-between px-4 pt-5 pb-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-[#f1ede4] hover:bg-[var(--ds-border)] transition-colors"
          >
    ...
    <div className="mt-4 space-y-3">
      ...
      >
        <DialogContent className={cn(
          "rounded-2xl border border-[#e8e3d9] bg-white shadow-xl p-6",
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full text-[14px] border border-[#e8e3d9] rounded-xl px-3.5 py-2.5 pr-10 focus:outline-none focus:ring-2 focus:ring-[var(--ds-primary)]/30 bg-white text-[#0f172a]"
                  placeholder="0"
                  autoFocus
                  onKeyDown={(e) => {
