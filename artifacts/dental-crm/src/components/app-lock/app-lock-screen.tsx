...
/** Let the PIN keypad paint before the system Face ID sheet. */
const BIOMETRIC_AFTER_PIN_MS = 280;
...
export function AppLockScreen() {
  ...
  useEffect(() => {
    // PIN keypad is visible first; prompt Face ID shortly after mount so the
    // system sheet appears on top of the lock UI (never during PWA splash).
    if (!config?.biometricEnabled || !biometricAvailable || lockedOut) return;
    const timer = window.setTimeout(() => {
      void tryBiometric();
    }, BIOMETRIC_AFTER_PIN_MS);
